...
# HYBRID NHL API
NHL_PBP = "https://api-web.nhle.com/v1/gamecenter/{}/play-by-play"
...
def fetch_scoreboard(date_str: str) -> list:
    ...
    for event in data.get("events", []):
        ...
        display_status = (
            ...
            else state_type.get("shortDetail", "")
        )
...
# =========================
@st.cache_data(ttl=30, show_spinner=False)
def fetch_nhl_plays(event_id: str):

    try:
        url = NHL_PBP.format(event_id)
        ...
        for p in data.get("plays", []):
            ...
            plays.append({
                ...
                "clock": p.get("timeInPeriod"),
                "type": p.get("typeDescKey", ""),
                "strength_name": strength.get("name"),
                "strength_code": strength.get("code"),
                "empty_net": details.get("emptyNet", False),
                "home_skaters": details.get("homeSkaters"),
                "away_skaters": details.get("awaySkaters"),
            })
...
def match_nhl_play(espn_play, nhl_plays):

    ep = espn_play.get("period", {}).get("number")

    ec = espn_play.get("clock", {}).get("displayValue")

    etype = (
        espn_play.get("type", {}).get("text") or ""
    ).lower()

    for np in nhl_plays:

        ntype = (np.get("type") or "").lower()
...
def get_parsed_plays(event_id: str) -> list:
    ...
    if (
        ...
        and st.session_state.cached_plays
    ):
    ...
    # NHL HYBRID
    nhl_plays = fetch_nhl_plays(event_id)
    ...
    for p in raw_plays:
        ...
        # NHL MATCH
        # =========================
        matched = match_nhl_play(p, nhl_plays)
        ...
        if matched:
            ...
            away_s = matched.get("away_skaters")
            home_s = matched.get("home_skaters")

            empty_net = matched.get("empty_net", False)

            if empty_net:

                if away_s and away_s > home_s:
                    home_g_in = False

                elif home_s and home_s > away_s:
                    away_g_in = False
            ...
            elif matched.get("strength_name"):

                strength_label = matched.get("strength_name")

        # =========================
        # PRIORITY 2 = ESPN FALLBACK
        # =========================
        else:

            sit = p.get("situation") or {}

            sit_code = str(
                sit.get("situationCode") or ""
            )
...
if st.session_state.view == "game":
    ...
    with nav_col3:
        ...
        refresh_time = st.session_state.last_refresh.strftime(
            "%H:%M:%S ET"
        )
    ...
    selected_periods = (
        ...
        if USE_PERIOD_FILTER else []
    )

    USE_TIME_FILTER = st.checkbox(
        "🕐 Filter by Actual Time (ET)",
        value=False
    )
    ...
    if USE_TIME_FILTER:
        ...
        def_start_date = (
            game_start_default.date()
            if game_start_default
            else ddate.today()
        )

        def_end_date = (
            game_end_default.date()
            if game_end_default
            else ddate.today()
        )

        def_start_time = (
            game_start_default.time()
            if game_start_default
            else dtime(19, 0)
        )

        def_end_time = (
            game_end_default.time()
            if game_end_default
            else dtime(23, 59)
        )
    ...
    USE_GOAL_FILTER = st.checkbox(
        "🚨 Goals Only",
        value=False
    )

    USE_PP_FILTER = st.checkbox(
        "⚡ Power Plays Only",
        value=False
    )

    USE_GP_FILTER = st.checkbox(
        "🥅 Empty Nets Only",
        value=False
    )
    ...
    if st.button("🚀 Apply Filters"):
        ...
        def passes(p):
            ...
            a_s = p.get("away_skaters")
            h_s = p.get("home_skaters")
            ...
            if (
                USE_PERIOD_FILTER
                and selected_periods
                and p["period_label"] not in selected_periods
            ):
                return False
            ...
            if USE_TIME_FILTER:
                ...
                if not (
                    START_DT <= p["wall_dt"] <= END_DT
                ):
                    return False

            if (
                USE_GOAL_FILTER
                and p["type_text"] != "Goal"
            ):
                return False
            ...
            if USE_GP_FILTER and (a_g and h_g):
                return False

            return True

        st.session_state.filtered_plays = [
            p for p in plays if passes(p)
        ]
    ...
    # DISPLAY FILTER INFO
    # =========================
    filters_applied = st.session_state.get(
        "filters_applied"
    )
    ...
    if filters_applied:
        ...
        if showing == 0:
            st.warning(
                "⚠️ No results found — please check the filters applied."
            )
            st.stop()
        ...
        if USE_PERIOD_FILTER:
            ...
            st.info(
                f"🏒 **Period filter:** "
                f"{', '.join(labels)} "
                f"— showing **{showing}** of **{total}** plays"
            )
        ...
        if USE_TIME_FILTER:
            st.info(
                ...
                f"→ "
                f"{END_DT.strftime('%Y-%m-%d %H:%M')} ET "
                f"— showing **{showing}** of **{total}** plays"
            )
        ...
        if USE_GOAL_FILTER:
            ...
            st.info(
                f"🚨 **Goals Only filter:** "
                f"{n_goals} goal(s) in game "
                f"— showing **{showing}** of **{total}** plays"
            )

        if USE_PP_FILTER:
            st.info(
                f"⚡ **Power Plays Only filter:** "
                f"showing **{showing}** of **{total}** plays"
            )

        if USE_GP_FILTER:
            st.info(
                f"🥅 **Empty Nets Only filter:** "
                f"showing **{showing}** of **{total}** plays"
            )
    ...
    for p in display_list:
        ...
        emoji = (
            "🚨"
            if p.get("type_text") == "Goal"
            else p.get("emoji", "🏒")
        )

        st.subheader(
            f"{emoji} {p.get('period_label')} "
            f"| ⏱️ {p.get('clock')}"
        )

        st.markdown(
            f"📊 **Score:** "
            f"{p.get('away_score')} - {p.get('home_score')}"
        )

        st.markdown(
            f"🎯 **Event:** {p.get('type_text')}"
        )

        st.markdown(
            f"⚖️ **Strength:** "
            f"`{p.get('strength', 'Unknown')}`"
        )
...
# ======================================================
    ...
    def handle_date_change():
        st.session_state.sched_date = (
            st.session_state.calendar_widget
        )
    ...
    if not games:
        ...
        st.info(
            f"No games scheduled for {formatted_date}."
        )
        ...
        for i, g in enumerate(games):
            ...
            ot_badge = (
                '<span class="sched-extra">OT</span>'
                if g["is_ot"]
                else ""
            )
            ...
            <div class="sched-team-row">
                ...
                <span class="sched-team-name">
                    {g['away_abbr']}
                </span>
                <span class="sched-score">
                    {g['away_score'] if has_started else ''}
                </span>
            </div>
            ...
            <div class="sched-team-row">
                ...
                <span class="sched-score">
                    {g['home_score'] if has_started else ''}
                </span>
            </div>
            ...
            with cols[i % 2]:
                ...
                with st.container(border=True):
                    ...
                    tooltip = (
                        ""
                        if has_started
                        else "Data will be available once the game starts."
                    )
